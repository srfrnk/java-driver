 ...
 *
 * <p>A {@code Mapper} object is obtained from a {@code MappingManager} using the {@link
...
public class Mapper<T> {
  ...
  private final ConcurrentMap<MapperQueryKey, ListenableFuture<PreparedStatement>> preparedQueries =
      new ConcurrentHashMap<MapperQueryKey, ListenableFuture<PreparedStatement>>();
  ...
  private static final EnumMap<Option.Type, Option> NO_OPTIONS =
      new EnumMap<Option.Type, Option>(Option.Type.class);
  ...
  ListenableFuture<PreparedStatement> getPreparedQueryAsync(
      QueryType type, Set<AliasedMappedProperty> columns, EnumMap<Option.Type, Option> options) {
    ...
    if (existingFuture == null) {
      ...
      if (old != null) {
        ...
        Futures.addCallback(
            ...
            new FutureCallback<PreparedStatement>() {
              @Override
  ...
  ListenableFuture<PreparedStatement> getPreparedQueryAsync(
      QueryType type, EnumMap<Option.Type, Option> options) {
   ...
   *
   * <p>This method is useful if you want to setup a number of options (tracing, consistency level,
   ...
   *
   * <p>Note: this method might block if the query is not prepared yet.
   *
  ...
  private ListenableFuture<BoundStatement> saveQueryAsync(
  ...
  private static <T> void setObject(
      BoundStatement bs,
      int i,
      T value,
   ...
   *
   * <p>This method is basically equivalent to: {@code
   * getManager().getSession().execute(saveQuery(entity))}.
   *
   * <p>Note: this method will block until the entity is fully saved.
   *
   * @param entity the entity to save.
   */
  ...
  private ListenableFuture<Void> submitVoidQueryAsync(ListenableFuture<BoundStatement> bsFuture) {
    ListenableFuture<ResultSet> rsFuture =
   ...
   *
   * <p>The values provided must correspond to the columns composing the PRIMARY KEY (in the order
   * of said primary key).
   *
   * <p>This method is useful if you want to setup a number of options (tracing, consistency level,
   ...
   *
   * <p>This method allows you to provide a suite of {@link Option} to include in the GET query.
  ...
  private ListenableFuture<BoundStatement> getQueryAsync(
      final List<Object> primaryKeys, final EnumMap<Option.Type, Option> options) {
   ...
   *
   * <p>Note: this method will block until the entity is fully fetched.
   *
   * @param objects the primary key of the entity to fetch, or more precisely the values for the
   *     columns of said primary key in the order of the primary key. Can be followed by {@link
   *     Option} to include in the DELETE query.
   ...
   *
   * <p>This method is basically equivalent to mapping the result of: {@code
   ...
   *
   * <p>This method is a shortcut that extract the PRIMARY KEY from the provided entity and call
   ...
   *
   * <p>This method allows you to provide a suite of {@link Option} to include in the DELETE query.
  ...
  private ListenableFuture<BoundStatement> deleteQueryAsync(
      T entity, EnumMap<Option.Type, Option> options) {
    List<Object> pks = new ArrayList<Object>();
  ...
  private ListenableFuture<BoundStatement> deleteQueryAsync(
      final List<Object> primaryKey, final EnumMap<Option.Type, Option> options) {
   ...
   *
   * <p>Note: this method will block until the entity is fully deleted.
   *
   * @param entity the entity to delete.
   */
   ...
   *
   * <p>If the query was user-generated (that is, passed directly to {@code session.execute()} or
   ...
   *
   * <p>If the query was generated by the mapper (for example with {@link #getQuery(Object...)}),
   ...
   *
   * <p>Use this method to map a {@link ResultSetFuture} that was not generated by the mapper (e.g.
  ...
  private static EnumMap<Option.Type, Option> toMapWithDefaults(
      Option[] options, EnumMap<Option.Type, Option> defaults) {
   ...
   *
   * <p>Options can be passed to individual operations:
   *
   * <pre>
   *     mapper.save(myObject, Option.ttl(3600));
   * </pre>
   *
   * <p>The mapper can also have defaults, that will apply to all operations that do not override
   ...
   * <p>
   *
   * <p>See the static methods in this class for available options.
   */
  public abstract static class Option {
     ...
     * no-op).
     *
     * <p>If the option's value is set as a bound variable, then this must be done at the given
     * index, and the method should return the new position.
     *
     * <p>Options may also affect the statement itself, not a bound variable; in that case, the
     ...
     *
     * <p>If the same option with different values produces different query strings, this should be
  ...
  private static class MapperQueryKey {
    ...
    MapperQueryKey(
        QueryType queryType,
        Set<AliasedMappedProperty> aliasedMappedProperties,
        EnumMap<Option.Type, Option> allOptions) {
