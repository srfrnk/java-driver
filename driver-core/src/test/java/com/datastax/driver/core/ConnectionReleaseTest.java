...
public class ConnectionReleaseTest extends ScassandraTestBase {
  ...
  /**
   * Validates that when a future is set that the stream associated with the future's request is
   * released. This prevents situations where a user may not be specifying a separate executor on a
   * callback/ transform to a ResultSetFuture, which is not recommended, causing executeAsync to
   * block in borrowConnection until stream ids become available.
   *
   ...
   *
   * <ol>
   *   <li>Sets # of connections per host to 1.
   *   <li>Sends MAX_STREAM_PER_CONNECTION-1 requests that take 10 seconds to execute.
   *   <li>Calls executeAsync to retrieve records from test1 with k=1.
   *   <li>Transforms executeAsync to take the 'c' column from the result and query test2. This is
   *       done without an executor to ensure the netty worker is used and has to wait for the
   *       function completion.
   *   <li>Asserts that the transformed future completes within pool timeout and the value is as
   *       expected.
   * </ol>
   *
   * @jira_ticket JAVA-666
   * @expected_result Are able to transform a Future without hanging in executeAsync as connection
   *     should be freed before the transform function is called.
  ...
  public void should_release_connection_before_completing_future() throws Exception {
    ...
    try {
          ...
          Futures.transformAsync(
              ...
              new AsyncFunction<ResultSet, ResultSet>() {
                ...
                public ListenableFuture<ResultSet> apply(ResultSet result) {
                  ...
                  // Execute async might hang if no streams are available.  This happens if the
                  // connection
                  // was not release.
      ...
      for (ResultSetFuture future : mockFutures) future.cancel(true);
