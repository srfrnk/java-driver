...
public class ReusedStreamIdTest extends CCMTestsSupport {
  ...
  /**
   * Ensures that if activity tied to future completion blocks netty io threads that this does not
   * cause the driver to possibly invoke the wrong handler for a response, as described in
   * JAVA-1179.
   *
   * <p>This is accomplished by setting up a 2 node cluster and setting a low read timeout (1
   * second). The test submits 10 concurrent requests repeatedly for up to 500 queries and on
   * completion of each request may block in a callback randomly (25% of the time) for 1 second,
   * causing a retry on the next host to trigger. If a new stream id is not allocated on the retry,
   * its possible it could use an already used stream id and cause the driver to invoke the wrong
   * handlers for a response. The test checks for this by ensuring that the column returned in a
   * response matches the one queried. If the column received does not match, the test fails. In
   * cases where this bug is present, it should be detected within 10 seconds.
   *
  ...
  public void should_not_receive_wrong_response_when_callbacks_block_io_thread() {
    ...
    try {
      ...
      List<ColumnMetadata> columnsToGrab =
          cluster().getMetadata().getKeyspace("system").getTable("local").getColumns();
      ...
      for (int i = 1; i <= iterations; i++) {
        try {
          if (errorTrigger.getCount() == 0) {
            fail(
                String.format(
                    "Error triggered at or before %d of %d requests after %dms.",
                    i, iterations, System.currentTimeMillis() - start));
          }
          semaphore.acquire();
          ...
          Futures.addCallback(
              ...
              new FutureCallback<ResultSet>() {
                ...
                public void onSuccess(ResultSet result) {
                  semaphore.release();
                  // Expect the column that you queried to be present, if its not we got the wrong
                  // response
                  // back.
                  int columnIndex = result.getColumnDefinitions().getIndexOf(column);
                  if (columnIndex == -1) {
                    logger.error(
                        "Got response without column {}, got columns {} from Host {}.",
                        column,
        ...
        if (i % (iterations / 10) == 0) {
          logger.info(
              "Submitted {} of {} requests. ({} completed successfully)",
              i,
              iterations,
              completed.get());
        }
      ...
      // Sanity check to ensure that at least some requests succeeded, we expect some failures if
      // both
