...
public abstract class Clause extends Utils.Appendeable implements Serializable {
  ...
  static class InClause extends AbstractClause {
    ...
    InClause(String name, Iterable<?> values) {
      super(name);
      if (values == null) throw new IllegalArgumentException("Missing values for IN clause");
    ...
    @Override
    boolean containsBindMarker() {
      for (Object value : values) if (Utils.containsBindMarker(value)) return true;
      return false;
    }
  }
  ...
  static class ContainsClause extends AbstractClause {
    ...
    ContainsClause(String name, Object value) {
      ...
      if (value == null) throw new IllegalArgumentException("Missing value for CONTAINS clause");
    }
  ...
  static class CompoundClause extends Clause {
    ...
    CompoundClause(Iterable<String> names, String op, Iterable<?> values) {
      ...
      if (this.names.size() != this.values.size())
        throw new IllegalArgumentException(
            String.format(
                "The number of names (%d) and values (%d) don't match",
                this.names.size(), this.values.size()));
    }
  ...
  static class CompoundInClause extends Clause {
    ...
    public CompoundInClause(Iterable<String> names, Iterable<?> valueLists) {
      if (valueLists == null) throw new IllegalArgumentException("Missing values for IN clause");
      if (names == null) throw new IllegalArgumentException("Missing names for IN clause");
        ...
        if (value instanceof Iterable) {
          ...
          if (tuple.size() != this.names.size()) {
            throw new IllegalArgumentException(
                String.format(
                    "The number of names (%d) and values (%d) don't match",
                    this.names.size(), tuple.size()));
          }
          this.valueLists.add(tuple);
        } else if (!(value instanceof BindMarker)) {
          throw new IllegalArgumentException(
              String.format(
                  "Wrong element type for values list, expected List or BindMarker, got %s",
