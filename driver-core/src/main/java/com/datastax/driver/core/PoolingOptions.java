 ...
 *
 * <p>The driver uses connections in an asynchronous manner, meaning that multiple requests can be
 * submitted on the same connection at the same time. Therefore only a relatively small number of
 * connections is needed. For each host, the driver uses a connection pool that may have a variable
 * size (it will automatically adjust to the current load).
 *
 * <p>With {@code ProtocolVersion#V2} or below, there are at most 128 simultaneous requests per
 * connection, so the pool defaults to a variable size. You will typically raise the maximum
 * capacity by adding more connections with {@link #setMaxConnectionsPerHost(HostDistance, int)}.
 *
 * <p>With {@code ProtocolVersion#V3} or above, there are up to 32768 requests per connection, and
 * the pool defaults to a fixed size of 1. You will typically raise the maximum capacity by allowing
 * more simultaneous requests per connection ({@link #setMaxRequestsPerConnection(HostDistance,
 * int)}).
 *
 * <p>All parameters can be separately set for {@code LOCAL} and {@code REMOTE} hosts ({@link
 * HostDistance}). For {@code IGNORED} hosts, no connections are created so these settings cannot be
 * changed.
 */
public class PoolingOptions {
   ...
   *
   * <p>Once a {@code PoolingOptions} object is associated to a {@link Cluster} and that cluster
   ...
   *
   * <p>The methods that may return this value are: {@link
   * #getCoreConnectionsPerHost(HostDistance)}, {@link #getMaxConnectionsPerHost(HostDistance)},
   * {@link #getNewConnectionThreshold(HostDistance)}, {@link
   ...
   *
   * <p>The map stores protocol versions in ascending order, and only the versions that introduced a
   * change are present. To find the defaults for a particular version, look for the highest key
   * that is less than or equal to that version, in other words:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * Once you've extracted the underlying map, use the keys {@code CORE_POOL_LOCAL_KEY}, {@code
   * MAX_POOL_LOCAL_KEY}, {@code CORE_POOL_REMOTE_KEY}, {@code MAX_POOL_REMOTE_KEY}, {@code
   * NEW_CONNECTION_THRESHOLD_LOCAL_KEY}, {@code NEW_CONNECTION_THRESHOLD_REMOTE_KEY}, {@code
   * MAX_REQUESTS_PER_CONNECTION_LOCAL_KEY} and {@code MAX_REQUESTS_PER_CONNECTION_REMOTE_KEY}.
   *
   * @see #UNSET
   */
  ...
  // The defaults for these fields depend on the protocol version, which is only known after control
  // connection initialization.
  // Yet if the user set them before initialization, we want to keep their values. So we use -1 to
  // mean "uninitialized".
   ...
   *
   * <p>For the provided {@code distance}, this corresponds to the number of connections initially
   * created and kept open to each host of that distance.
   *
   * <p>The default value is:
   *
   * <ul>
   *   <li>with {@code ProtocolVersion#V2} or below: 2 for {@code LOCAL} hosts and 1 for {@code
   *       REMOTE} hosts.
   *   <li>with {@code ProtocolVersion#V3} or above: 1 for all hosts.
   * </ul>
   *
   ...
   * @throws IllegalArgumentException if {@code distance == HostDistance.IGNORED}, or if {@code
   *     newCoreConnections} is greater than the maximum value for this distance.
   ...
   *
   * <p>For the provided {@code distance}, this corresponds to the maximum number of connections
   * that can be created per host at that distance.
   *
   * <p>The default value is:
   *
   * <ul>
   *   <li>with {@code ProtocolVersion#V2} or below: 8 for {@code LOCAL} hosts and 2 for {@code
   *       REMOTE} hosts.
   *   <li>with {@code ProtocolVersion#V3} or above: 1 for all hosts.
   * </ul>
   *
   ...
   * @return this {@code PoolingOptions}.
   * @throws IllegalArgumentException if {@code distance == HostDistance.IGNORED}, or if {@code
   *     newMaxConnections} is less than the core value for this distance.
   * @see #setConnectionsPerHost(HostDistance, int, int)
   */
   ...
   *
   * <p>This is a convenience method that is equivalent to calling {@link
   * #setCoreConnectionsPerHost(HostDistance, int)} and {@link
   ...
   *
   * <p>A new connection gets created if:
   *
   * <ul>
   *   <li>N connections are open
   *   <li><tt>N < {@link #getMaxConnectionsPerHost(HostDistance)}</tt>
   *   <li>the number of active requests is more than <tt>(N - 1) * {@link
   *       #getMaxRequestsPerConnection(HostDistance)} + {@link
   *       #getNewConnectionThreshold(HostDistance)}</tt>
   * </ul>
   *
   * In other words, if all but the last connection are full, and the last connection is above this
   * threshold.
   *
   * <p>The default value is:
   *
   * <ul>
   *   <li>with {@code ProtocolVersion#V2} or below: 100 for all hosts.
   *   <li>with {@code ProtocolVersion#V3} or above: 800 for {@code LOCAL} hosts and 200 for {@code
   *       REMOTE} hosts.
   * </ul>
   *
   ...
   * @return this {@code PoolingOptions}.
   * @throws IllegalArgumentException if {@code distance == HostDistance.IGNORED}, or if {@code
   *     maxSimultaneousRequests} is not in range, or if {@code newValue} is less than the minimum
   ...
   *
   * <ul>
   *   <li>with {@code ProtocolVersion#V2} or below: 128 for all hosts (there should not be any
   *       reason to change this).
   *   <li>with {@code ProtocolVersion#V3} or above: 1024 for {@code LOCAL} hosts and 256 for {@code
   *       REMOTE} hosts. These values were chosen so that the default V2 and V3 configuration
   ...
   *       values. If you have more than one connection per host, consider also adjusting {@link
   ...
   * @return this {@code PoolingOptions}.
   * @throws IllegalArgumentException if {@code distance == HostDistance.IGNORED}, or if {@code
   *     newMaxConnections} is not within the allowed range.
   */
   ...
   *
   * <p>The order of magnitude should be a few minutes (the default is 120 seconds). The timeout
   ...
   *
   * <p>This option works in concert with {@link #setMaxQueueSize(int)} to determine what happens if
   * the driver tries to borrow a connection from the pool but none is available:
   *
   * <ul>
   *   <li>if either option is set to zero, the attempt is rejected immediately;
   *   <li>else if more than {@code maxQueueSize} requests are already waiting for a connection, the
   *       attempt is also rejected;
   *   <li>otherwise, the attempt is enqueued; if a connection becomes available before {@code
   *       poolTimeoutMillis} has elapsed, then the attempt succeeds, otherwise it is rejected.
   * </ul>
   *
   * If the attempt is rejected, the driver will move to the next host in the {@link
   * com.datastax.driver.core.policies.LoadBalancingPolicy#newQueryPlan(String, Statement)} query
   * plan}.
   *
   * <p>The default is 5 seconds. If this option is set to zero, the driver won't wait at all.
   *
   ...
   *
   * <p>This option works in concert with {@link #setPoolTimeoutMillis(int)} to determine what
   * happens if the driver tries to borrow a connection from the pool but none is available:
   *
   * <ul>
   *   <li>if either options is set to zero, the attempt is rejected immediately;
   *   <li>else if more than {@code maxQueueSize} requests are already waiting for a connection, the
   *       attempt is also rejected;
   *   <li>otherwise, the attempt is enqueued; if a connection becomes available before {@code
   *       poolTimeoutMillis} has elapsed, then the attempt succeeds, otherwise it is rejected.
   ...
   * plan}.
   *
   * <p>The default value is {@value DEFAULT_MAX_QUEUE_SIZE}. If this option is set to zero, the
   * driver will never enqueue requests.
   *
   ...
   * it's still alive.
   *
   * <p>This is an application-level keep-alive, provided for convenience since adjusting the TCP
   * keep-alive might not be practical in all environments.
   *
   * <p>This option should be set higher than {@link SocketOptions#getReadTimeoutMillis()}.
   *
   * <p>The default value for this option is 30 seconds.
   *
   * @param heartbeatIntervalSeconds the new value in seconds. If set to 0, it will disable the
   *     feature.
   * @return this {@code PoolingOptions}
   ...
   *
   * <p>Connections are open in a completely asynchronous manner. Since initializing the transport
   ...
   *
   * <p><b>This is an advanced option, which should be rarely needed in practice.</b> It defaults to
   * Guava's {@code MoreExecutors.sameThreadExecutor()}, which results in running the
   ...
   * executor is if you use authentication with a custom {@link
   * com.datastax.driver.core.Authenticator} implementation that performs blocking calls.
   *
  ...
  /**
   * Requests the driver to re-evaluate the {@link HostDistance} (through the configured {@link
   * com.datastax.driver.core.policies.LoadBalancingPolicy#distance}) for every known hosts and to
   * drop/add connections to each hosts according to the computed distance.
   *
   * <p>Note that, due to backward compatibility issues, this method is not interruptible. If the
   * caller thread gets interrupted, the method will complete and only then re-interrupt the thread
   * (which you can check with {@code Thread.currentThread().isInterrupted()}).
   */
  ...
  private void checkRequestsPerConnectionRange(
    ...
    // If we don't know the protocol version yet, use the highest possible upper bound, this will
    // get checked again when possible
    int max =
