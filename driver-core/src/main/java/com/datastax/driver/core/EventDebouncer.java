 ...
 *
 * <p>This class accumulates received events, and delivers them when either: - no events have been
...
abstract class EventDebouncer<T> {
  ...
  private final AtomicReference<DeliveryAttempt> immediateDelivery =
      new AtomicReference<DeliveryAttempt>(null);
  private final AtomicReference<DeliveryAttempt> delayedDelivery =
      new AtomicReference<DeliveryAttempt>(null);
  ...
  void deliverEvents() {
    ...
    if (eventCount.get() > 0) scheduleDelayedDelivery();
  }
  ...
  class DeliveryAttempt extends ExceptionCatchingRunnable {
    ...
    void cancel() {
      if (deliveryFuture != null) deliveryFuture.cancel(true);
    }

    void executeNow() {
      if (state != State.STOPPED) deliveryFuture = executor.submit(this);
    }
  ...
  interface DeliveryCallback<T> {
    ...
    /**
     * Deliver the given list of events. The given list is a private copy and any modification made
     * to it has no side-effect; it is also guaranteed not to be null nor empty.
     *
