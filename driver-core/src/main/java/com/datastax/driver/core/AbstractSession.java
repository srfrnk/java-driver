...
public abstract class AbstractSession implements Session {
   ...
   * @param customPayload the custom payload to send along the query, or {@code null} if no payload
   *     is to be sent
   ...
   *
   * <p>This method is called from all the synchronous methods of this class to prevent deadlock
   * issues.
   *
   * <p>User code extending this class can also call this method at any time to check if any code
   * making blocking calls is being wrongly executed on a Netty I/O thread.
   *
   * <p>Note that the check performed by this method has a small overhead; if that is an issue,
   * checks can be disabled by setting the System property {@code
   * com.datastax.driver.CHECK_IO_DEADLOCKS} to {@code false}.
   *
   * @throws IllegalStateException if the current thread is one of the Netty I/O thread used by the
   *     driver.
   */
  public void checkNotInEventLoop() {
    Connection.Factory connectionFactory = getCluster().manager.connectionFactory;
    if (!CHECK_IO_DEADLOCKS || connectionFactory == null) return;
